...
def refresh_opensea_metadata(token_id: int, chain_id: int) -> None:
    ...
    if chain is None:
        raise ValueError(f"Chain ID {chain_id} not recognized")
    address = NFT_ADDRESS
    url = f'https://api.opensea.io/api/v2/chain/{chain}/contract/{address}/nfts/{token_id}/refresh'
    logger.debug(f"Refreshing OpenSea metadata with URL: {url}")
...
def handle_task(task: ExternalTask) -> TaskResult:
    ...
    if not tx_hash or not art_id or not chain_id:
        logger.error("transactionHash or gen_art_id or chain_id is missing")
        return task.failure(
            "MISSING_VARIABLES",
            "transactionHash or gen_art_id or chain_id is missing",
            max_retries=3,
            retry_timeout=5000,
        )
    try:
