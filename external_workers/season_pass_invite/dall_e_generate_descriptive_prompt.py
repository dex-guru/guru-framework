...
async def name_description_based_of_vision_description(image_type, vision_description, retries: int = 2) -> tuple[
    ...
    - tweet (a cool Twitter post that people would love to share about minting their unique NFT. The post must be 140 characters max and include @pixelpactai. If the length is less than 140 characters, add a brief summary of the full story to reach the character limit)
...
async def post_based_of_video_description(image_type, vision_description, retries: int = 2) -> tuple[bool, dict]:
    ...
    post_prompt = "Maximum 200 symbols. Generate a tweet announcing the completion of a new digital artwork on Guru Network and the excitement of minting it soon. Use hashtags #NFTCommunity, #DigitalArt, and #GuruNetwork. Mention @pixelpactai. put it in gen_post field."
    # Call the OpenAI API
    try:
        response = await client.chat.completions.create(
            model="gpt-3.5-turbo-1106",
