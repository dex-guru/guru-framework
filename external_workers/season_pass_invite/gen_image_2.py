...
from season_pass_invite.gen_img_new import BlendImagesProcessor
...
async def get_text_descriptions(img_picture, art_name, art_description, image_type):
    # Task handling code with added logging
    try:

        status, vision_result = await describe_image_with_openai_vision(img_picture, art_name, art_description, image_type)
        if not status:
            return False, None
        status, name_description = await name_description_based_of_vision_description(image_type, vision_result)
        if not status:
            return False, None

        status, gen_post = await post_based_of_video_description(image_type, vision_result)
        if not status:
            return False, None

        return name_description.get("name"), name_description.get("description"), gen_post.get("gen_post")
...
async def generate_and_upload_image(src1_art_id: str, src2_art_id: str):
    ...
    return generated_art, name, description, gen_post
...
def handle_task(task: ExternalTask) -> TaskResult:
    ...
    try:
        generated_art, name, description, gen_post = loop.run_until_complete(generate_and_upload_image(src1_art_id,
                                                                                                       src2_art_id))
        if not generated_art:
            raise Exception("Empty generated Art")
        variables["gen_post"] = gen_post
        variables["gen_token_name"] = name
        variables["gen_token_tags"] = "robot, rocket"
